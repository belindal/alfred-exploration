...
class ALFREDDataloader(DataLoader):
    def __init__(self, args, vocab, dataset, split_type, batch_size, sep_actions: bool):
        ...
        dataset = [self.load_task_json(task, args.data, args.pp_folder) for task in dataset]
            ...
            for ex in tqdm(self.dataset):
                ...
                for subgoal_idx, subgoal in enumerate(ex['num']['lang_instr']):
                    ...
                    new_ex = {
                        ...
                        'all_subgoals': ex['num']['lang_instr'],
    ...
    def collate_fn(self, batch):
        ...
        for item in batch:
            # 'goal', 'curr_subgoal', 'prev_subgoals', 'all_subgoals', 'all_states', 'all_actions', 'all_actions_mask'
            # if gt_alignment:
